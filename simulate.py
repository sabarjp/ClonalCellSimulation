...
class Cell:
	...
	def __repr__(self):
		return 'Cell(dr=%s, mr=%s, err=%s, mut=%s, L=%s, rep=%s, p53=%s, ttl=%s, g=%s)' % (
			    self.drug_resistance, self.mitosis_rate, 
			    self.new_errors, self.dynasty_mutations, self.life, 
			    self.repair_success, self.p53_effectiveness, self.time_to_live,
			    self.generational_number)

	def gain_energy(self):
		self.excess_energy = self.excess_energy + max(normalvariate(self.AVERAGE_ENERGY_PER_TICK, self.AVERAGE_ENERGY_PER_TICK / 4.0), 0.0)

	def gain_life(self):
		self.life = self.life + min(max(normalvariate(self.AVERAGE_LIFE_PER_TICK, self.AVERAGE_LIFE_PER_TICK / 4.0), 0.0), 150)

	def has_energy_to_divide(self):
		return self.excess_energy >= self.ENERGY_NEEDED_TO_DIVIDE

	def has_life_to_divide(self):
		return self.life > self.LIFE_NEEDED_TO_DIVIDE
	...
	def mutate_non_lethal_bad(self):
		...
		if(dice_roll < 250):
			self.drug_resistance = max(self.drug_resistance - randint(1, 15), 0)
		elif(dice_roll < 500):
			self.mitosis_rate = self.mitosis_rate + randint(1, 2)
		elif(dice_roll < 750):
			self.life = max(self.life - randint(1, int(self.life) + 1), 0)
		elif(dice_roll < 900):
			self.repair_success = max(self.repair_success - uniform(0.0001, 0.0050), 0)
		elif(dice_roll < 960):
			self.max_time_to_live = max(self.max_time_to_live - randint(1, 3), 1)
		elif(dice_roll < 999):
			self.can_divide = False
		else:
			self.p53_effectiveness = max(self.p53_effectiveness - uniform(0.01, 1.0), 0)
	...
	def mutate_good(self):
		...
		if(dice_roll < 250):
			...
			self.max_time_to_live = max(self.max_time_to_live + randint(1, 2), 0)
		elif(dice_roll < 999):
			self.repair_success = min(max(self.repair_success + uniform(0.0001, 0.0050), 0.0), 0.99999)
		else:
			self.p53_effectiveness = min(self.p53_effectiveness + uniform(0.01, 0.3), 2.0)

	def damage(self):
		dna_errors_max = int(max(normalvariate(self.ERRORS_PER_TICK, self.ERRORS_PER_TICK / 4), 0))
	...
	def repair(self):
		...
		self.new_errors = int(max(normalvariate(expected_errors, expected_errors / 4), 0.0))

		# There need to be errors and the p53 gene needs to be working for repair
		if(self.new_errors > 0 and uniform(0.0, 1.0) <= self.p53_effectiveness * self.new_errors):
			dice_roll = randint(1,1000)
	...
	def mutate(self):
		...
		dna_mutations = int(max(normalvariate(dna_mutations_expected_value, dna_mutations_expected_value / 4), 0.0))
		...
		self.is_expressing_p53 = (self.p53_effectiveness * self.dynasty_mutations) > 1
		self.new_errors = 0

	def divide(self):
		# The idea here is that the cell requires a store of energy and
		# must be healthy enough to divide.  This acts as strong selection
		# against mutation since most mutations affect fitness in a negative
		# manner.  Without this, cells tend to go wild.
		if(self.has_energy_to_divide() and self.has_life_to_divide()):
			self.excess_energy = self.excess_energy - self.ENERGY_NEEDED_TO_DIVIDE
			self.life = self.life - self.LIFE_NEEDED_TO_DIVIDE
...
class Population:
	...
	def poison(self, amount_of_poison):
		for cell in self.cell_collection:
			...
			poison_strength_after_resistance = (amount_of_poison * 1.0) - (cell.drug_resistance * normalvariate(1, 0.12))

			if(poison_strength_after_resistance > 0):
				effectiveness = max((poison_strength_after_resistance / 2.0), 0.0)
			else:
				effectiveness = 0

			#print 'poison: bd:', base_damage, ' str: ', poison_strength_after_resistance, ' effect: ', effectiveness, ' = ', base_damage * effectiveness
	...
	def tick(self):
		...
		# kill off anything dead from in between ticks
		self.cell_collection[:] = [x for x in self.cell_collection if x.life > 0 and not(x.is_signaling_apoptosis)]
		...
		for cell in self.cell_collection:
			...
			if(len(self.cell_collection) < 100):
				# time for dividing
				if(self.current_tick % cell.mitosis_rate == 0 and cell.has_energy_to_divide() and cell.has_life_to_divide()):
